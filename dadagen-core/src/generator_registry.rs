...
impl GeneratorRegistry {
    ...
    fn register_standard_generators(&mut self) {
        ...
        self.register_metadata(GeneratorTypeMetadata {
            ...
            description: "Generates random strings with configurable length, character sets, and case".to_string(),
            parameters: vec![
                ParameterMetadata {
                ...
                ParameterMetadata {
                    ...
                    description: "Character set: Alpha, Numeric, AlphaNumeric, Hex, Ascii, Base64".to_string(),
                },
                ParameterMetadata {
                    name: "case".to_string(),
        ...
        self.register_metadata(GeneratorTypeMetadata {
            ...
            description: "Generates random numbers (integer or decimal) with optional distributions".to_string(),
            parameters: vec![
                ParameterMetadata {
                    name: "min".to_string(),
        ...
        self.register_metadata(GeneratorTypeMetadata {
            ...
            description: "Generates random boolean values with configurable probability".to_string(),
            parameters: vec![
                ParameterMetadata {
        ...
        self.register_metadata(GeneratorTypeMetadata {
            ...
            description: "Generates sequential numbers with configurable start and step".to_string(),
            parameters: vec![
                ParameterMetadata {
        ...
        self.register_metadata(GeneratorTypeMetadata {
            ...
            description: "Selects values from data lists with optional discriminator filtering".to_string(),
            parameters: vec![
                ParameterMetadata {
                    name: "name".to_string(),
                    param_type: "String".to_string(),
    ...
    pub fn register_metadata(&mut self, metadata: GeneratorTypeMetadata) {
        self.metadata.insert(metadata.generator_type.clone(), metadata);
    }
    ...
    pub fn create_for_field(&self, document: &DslDocument, field_name: &str) -> Result<Box<dyn DataGenerator>> {
        // Find the field in the document
        let field = document.fields.iter()
    ...
    pub fn generate_documentation(&self) -> String {
        ...
        doc.push_str("This document describes all available generator types and their parameters.\n\n");
        ...
        for metadata in types {
            ...
            doc.push_str(&format!("**Can have dependencies:** {}\n\n", 
                if metadata.can_have_dependencies { "Yes" } else { "No" }));
    ...
    pub fn clear_cache(&self) -> Result<()> {
        let mut cache = self.generator_cache.write()
            .map_err(|e| DadagenError::context_error(format!("Failed to acquire write lock: {}", e)))?;
        cache.clear();
        Ok(())
    }
}
...
mod tests {
    ...
    fn test_create_generator_from_ast() {
        ...
        let gen = registry.create_from_ast(&ast_gen).unwrap();
        assert_eq!(gen.generator_type(), GeneratorType::String);
        
        let context = Context::new();
        let result = gen.generate(&context).unwrap();
    ...
    fn test_all_generator_types_integration() {
        ...
        let string_gen = registry.create_from_ast(&Generator::String(StringGenerator {
            length: Some(5),
            min_length: None,
            max_length: None,
        ...
        let number_gen = registry.create_from_ast(&Generator::Number(NumberGenerator {
            min: Some(10.0),
            max: Some(20.0),
        ...
        let bool_gen = registry.create_from_ast(&Generator::Boolean(BooleanGenerator {
        ...
        let choice_gen = registry.create_from_ast(&Generator::Choice(ChoiceGenerator {
        ...
        let counter_gen = registry.create_from_ast(&Generator::Counter(CounterGenerator {
            start: Some(100),
            step: Some(5),
            span: None,
        })).unwrap();
        ...
        let template_gen = registry.create_from_ast(&Generator::Template(TemplateGenerator {
        ...
        let regex_gen = registry.create_from_ast(&Generator::Regex(RegexGenerator {
        ...
        let address_gen = registry.create_from_ast(&Generator::Address(AddressGenerator {
        ...
        // Test Name generator
        let name_gen = registry.create_from_ast(&Generator::Name(NameGenerator {
        ...
        let gender_gen = registry.create_from_ast(&Generator::Gender(GenderGenerator {
            span: None,
        })).unwrap();
