...
impl ContextPool {
    ...
    pub fn with_seed(capacity: usize, seed: u64) -> Self {
        debug!(
            "Creating seeded context pool with capacity: {} and seed: {}",
            capacity, seed
        );
    ...
    pub fn acquire(&self) -> Result<Context> {
        ...
        let mut available = self.available.lock().map_err(|e| {
            DadagenError::generation_error(format!("Failed to acquire pool lock: {}", e))
        })?;
        ...
        let mut total = self.total_contexts.lock().map_err(|e| {
            DadagenError::generation_error(format!("Failed to acquire count lock: {}", e))
        })?;
        ...
        if *total < self.max_size {
            *total += 1;
            ...
            debug!(
                "Creating new context (total: {})",
                *self.total_contexts.lock().unwrap()
            );
        ...
        Err(DadagenError::generation_error(
            "Context pool exhausted - all contexts in use",
        ))
    }
    ...
    pub fn total_count(&self) -> Result<usize> {
        let total = self.total_contexts.lock().map_err(|e| {
            DadagenError::generation_error(format!("Failed to acquire count lock: {}", e))
        })?;
        Ok(*total)
    }
