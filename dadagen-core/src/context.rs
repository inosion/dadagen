...
impl fmt::Debug for Context {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        let metadata = self.generation_metadata.read().ok().and_then(|m| Some(m.clone()));
...
impl Context {
        ...
        T: Clone + Send + Sync + fmt::Debug + 'static,
    {
        ...
        let mut state = self.data_field_state.write()
            .map_err(|e| DadagenError::context_error(
                format!("Failed to acquire write lock for field state: {}", e)
            ))?;
        ...
        T: Clone + Sync + Send + 'static,
    {
        ...
        let result = {
            let state = self.data_field_state.read()
                .map_err(|e| DadagenError::context_error(
                    format!("Failed to acquire read lock for field state: {}", e)
                ))?;
    ...
    pub fn increment_iteration(&self) -> Result<u64> {
        let mut iter = self.current_iter.write()
            .map_err(|e| DadagenError::context_error(
                format!("Failed to acquire write lock for iteration counter: {}", e)
            ))?;
    ...
    pub fn current_iteration(&self) -> Result<u64> {
        let iter = self.current_iter.read()
            .map_err(|e| DadagenError::context_error(
                format!("Failed to acquire read lock for iteration counter: {}", e)
            ))?;
    ...
    pub fn register_field_dependency(&self, field: String, depends_on: Vec<String>) -> Result<()> {
        let mut deps = self.field_dependencies.write()
            .map_err(|e| DadagenError::context_error(
                format!("Failed to acquire write lock for dependencies: {}", e)
            ))?;
    ...
    pub fn get_field_dependencies(&self, field: &str) -> Result<Vec<String>> {
        let deps = self.field_dependencies.read()
            .map_err(|e| DadagenError::context_error(
                format!("Failed to acquire read lock for dependencies: {}", e)
            ))?;
        ...
        F: FnOnce(&mut GenerationMetadata)
    {
        let mut metadata = self.generation_metadata.write()
            .map_err(|e| DadagenError::context_error(
                format!("Failed to acquire write lock for metadata: {}", e)
            ))?;
    ...
    pub fn get_metadata(&self) -> Result<GenerationMetadata> {
        let metadata = self.generation_metadata.read()
            .map_err(|e| DadagenError::context_error(
                format!("Failed to acquire read lock for metadata: {}", e)
            ))?;
...
mod tests {
    ...
    fn test_field_required() {
        ...
        context.insert_field_state("exists".to_string(), "value".to_string()).unwrap();
    ...
    fn test_context_snapshot() {
        ...
        context.insert_field_state("name".to_string(), "Alice".to_string()).unwrap();
        context.insert_field_state("age".to_string(), 30i32).unwrap();
        context.register_field_dependency(
            "email".to_string(),
            vec!["name".to_string()]
        ).unwrap();
    ...
    fn test_deep_clone() {
        ...
        context.insert_field_state("new_field".to_string(), "value".to_string()).unwrap();
        assert!(cloned.get_field_state::<String>("new_field").unwrap().is_none());
    }
    ...
    fn test_shallow_clone() {
        ...
        context.insert_field_state("shared".to_string(), "value".to_string()).unwrap();
        assert_eq!(cloned.get_field_state::<String>("shared").unwrap(), Some("value".to_string()));
    }
    ...
    fn test_anyclone_trait() {
        ...
        assert_eq!(
            *cloned_int.as_any().downcast_ref::<i64>().unwrap(),
            42
        );
        
        let cloned_bool = bool_val.clone();
        assert_eq!(
            *cloned_bool.as_any().downcast_ref::<bool>().unwrap(),
            true
        );
