...
impl Generator<String> for StringGenerator {
    fn generate(&self, _context: &Context) -> Result<String> {
        ...
        if chars.is_empty() {
            return Err(DadagenError::GenerationError { message: "Character set is empty".to_string() });
        }
        ...
        if min_len > max_len {
            return Err(DadagenError::GenerationError { message: format!("Invalid length range: min ({}) must be <= max ({})", min_len, max_len) });
        }
...
impl PatternGenerator {
    ...
    fn generate_from_pattern(&self, pattern: &str, context: &Context, rng: &mut StdRng) -> Result<String> {
            ...
            match ch {
                '#' if chars.peek() == Some(&'{') => {
                    ...
                    if let Ok(Some(value)) = context.get_field_state::<String>(&field_name) {
                        ...
                        return Err(DadagenError::GenerationError { message: format!("Field '{}' not found in context", field_name) });
                    }
                },
                '{' => {
                    // Pattern specifier
                    ...
                    let generated = match spec.as_str() {
                        ...
                        spec if spec.chars().all(|c| c.is_ascii_digit()) => {
                            let len: usize = spec.parse().map_err(|_| 
                                DadagenError::GenerationError { message: format!("Invalid length specifier: {}", spec) }
                            )?;
                            
                            let charset = CharacterSet::AlphaNumeric.to_chars();
                            (0..len)
                                .map(|_| *charset.choose(rng).unwrap())
                                .collect()
                        },
                        _ => return Err(DadagenError::GenerationError { message: format!("Unknown pattern specifier: {}", spec) }),
                    };
...
impl Generator<String> for ListGenerator {
    fn generate(&self, _context: &Context) -> Result<String> {
        if self.options.is_empty() {
            return Err(DadagenError::GenerationError { message: "No options available for list generator".to_string() });
        }
