...
impl GeneratorRegistry {
    ...
    pub fn register_factory(
        &mut self,
        name: &str,
        factory: Arc<dyn GeneratorFactory + Send + Sync>,
    ) {
    ...
    pub fn create_generator(&self, type_name: &str, config: &str) -> Result<BoxedGenerator> {
        if let Some(factory) = self.registry.get(type_name) {
            ...
            Err(DadagenError::GenerationError {
                message: format!("Unknown generator type: {}", type_name),
            })
        }
    }
...
impl GeneratorFactory for StringGeneratorFactory {
    fn create(&self, _config: &str) -> Result<BoxedGenerator> {
        ...
        let generator = crate::generators::StringGenerator::new("string".to_string());
        Ok(Box::new(GeneratorWrapper::new(generator)))
    }
    fn metadata(&self) -> GeneratorMetadata {
        GeneratorMetadata {
...
impl GeneratorFactory for IntegerGeneratorFactory {
    fn create(&self, _config: &str) -> Result<BoxedGenerator> {
        let generator = crate::generators::IntegerGenerator::new("integer".to_string());
        Ok(Box::new(GeneratorWrapper::new(generator)))
    }
    fn metadata(&self) -> GeneratorMetadata {
        GeneratorMetadata {
...
impl GeneratorFactory for FloatGeneratorFactory {
    fn create(&self, _config: &str) -> Result<BoxedGenerator> {
        let generator = crate::generators::FloatGenerator::new("float".to_string());
        Ok(Box::new(GeneratorWrapper::new(generator)))
    }
    fn metadata(&self) -> GeneratorMetadata {
        GeneratorMetadata {
...
impl GeneratorFactory for BooleanGeneratorFactory {
    fn create(&self, _config: &str) -> Result<BoxedGenerator> {
        ...
        let generator =
            crate::generators::IntegerGenerator::new("boolean".to_string()).with_range(0, 1);
        Ok(Box::new(GeneratorWrapper::new(generator)))
    }
    fn metadata(&self) -> GeneratorMetadata {
        GeneratorMetadata {
...
impl GeneratorFactory for TemplateGeneratorFactory {
    fn create(&self, config: &str) -> Result<BoxedGenerator> {
        ...
        let generator = crate::generators::TemplateGenerator::new("template".to_string(), template);
        Ok(Box::new(GeneratorWrapper::new(generator)))
    }
    fn metadata(&self) -> GeneratorMetadata {
        GeneratorMetadata {
...
impl RegistryBuilder {
    ...
    pub fn with_standard_generators(mut self) -> Self {
        self.registry
            .register_factory("string", Arc::new(StringGeneratorFactory));
        self.registry
            .register_factory("integer", Arc::new(IntegerGeneratorFactory));
        self.registry
            .register_factory("float", Arc::new(FloatGeneratorFactory));
        self.registry
            .register_factory("boolean", Arc::new(BooleanGeneratorFactory));
        self.registry
            .register_factory("template", Arc::new(TemplateGeneratorFactory));
        self
    }
...
mod tests {
    ...
    fn test_registry_thread_safety() {
        ...
        let handles: Vec<_> = (0..10)
            .map(|_| {
                ...
                thread::spawn(move || {
                    let context = Context::new();
                    let generator = registry.create_generator("string", "").unwrap();
                    let value = generator.generate_value(&context).unwrap();
        ...
        for handle in handles {
            assert!(
                handle.join().unwrap(),
                "Thread should successfully generate values"
            );
        }
    }
    ...
    fn test_registry_builder() {
        let registry = RegistryBuilder::new().with_standard_generators().build();
        ...
        let template_gen = registry
            .create_generator("template", "Hello-{random}")
            .unwrap();
    ...
    fn test_template_generator_with_config() {
        ...
        let custom_gen = registry
            .create_generator("template", "User-{id}-Profile")
            .unwrap();
    ...
    fn test_generator_dependencies() {
        ...
        let template_gen = registry
            .create_generator("template", "Hello {{user_name}}, your ID is {{user_id}}")
            .unwrap();
        ...
        let simple_template_gen = registry
            .create_generator("template", "Simple static text")
            .unwrap();
