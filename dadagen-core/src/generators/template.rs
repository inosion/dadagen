            ...
            if let Some(args) = cap.get(2) {
                ...
                for arg_cap in arg_field_regex.captures_iter(args.as_str()) {
                    if let Some(field) = arg_cap.get(1) {
                        ...
                        if !["random", "choose", "format", "if", "gen"]
    ...
    fn process_template(&self, template: &str, context: &Context) -> Result<String> {
        ...
        if result.contains("{{ERROR:") {
            return Err(DadagenError::GenerationError {
                message: format!("Template processing failed: {}", result),
            });
        }
    ...
    fn execute_function(&self, func_name: &str, args: &str, context: &Context) -> Result<String> {
        match func_name {
            "random" => {
                ...
                if parts.len() != 2 {
                    return Err(DadagenError::GenerationError {
                        message: "random() requires exactly 2 arguments: min, max".to_string(),
                    });
                }

                let min: i64 = parts[0]
                    .parse()
                    .map_err(|_| DadagenError::GenerationError {
                        message: "Invalid min value for random()".to_string(),
                    })?;
                let max: i64 = parts[1]
                    .parse()
                    .map_err(|_| DadagenError::GenerationError {
                        message: "Invalid max value for random()".to_string(),
                    })?;

                let generator =
                    IntegerGenerator::new("temp_random".to_string()).with_range(min, max);
            ...
            "choose" => {
                ...
                if options.is_empty() {
                    return Err(DadagenError::GenerationError {
                        message: "choose() requires at least one option".to_string(),
                    });
                }
            ...
            "format" => {
                ...
                if parts.len() != 2 {
                    return Err(DadagenError::GenerationError {
                        message: "format() requires exactly 2 arguments: pattern, value"
                            .to_string(),
                    });
                }
                ...
                match context.get_field_state::<String>(value_name) {
                    ...
                    Ok(None) => Err(DadagenError::GenerationError {
                        message: format!("Field '{}' not found for format()", value_name),
                    }),
                    Err(e) => Err(e),
                }
            }
            ...
            _ => Err(DadagenError::GenerationError {
                message: format!("Unknown template function: {}", func_name),
            }),
        }
    }
...
impl Generator<String> for CompositeGenerator {
    fn generate(&self, context: &Context) -> Result<String> {
        ...
        let template_gen =
            TemplateGenerator::new(format!("{}_template", self.name), self.template.clone());
