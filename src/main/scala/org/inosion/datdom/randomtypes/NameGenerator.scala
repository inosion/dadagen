...
import org.inosion.datdom.{ListConfigSupport, ListManager, RandomUtil, Context}
...
case class SurnameGenerator(name:String,
                            listName:String=SurnameGenerator.keyNameMedium) extends DataGenerator[String] {

  ListConfigSupport.importConfigListData(listName)

  override def internalGenerate(context: Context): String = ListManager.getRandomValue(listName)
