...
fn expand_integer_generator(call: &GeneratorCall) -> syn::Result<TokenStream> {
    ...
    for param in &call.params {
        match param.name.to_string().as_str() {
            "min" | "max" => {
                if let (Some(min), Some(max)) =
                    (find_param_value(call, "min"), find_param_value(call, "max"))
                {
                    generator = quote! {
        ...
        Expr::Lit(expr_lit) => {
            if let syn::Lit::Str(lit_str) = &expr_lit.lit {
                let charset = match lit_str.value().as_str() {
                    ...
                    "alphanumeric" => {
                        quote! { dadagen_core::generators::CharacterSet::AlphaNumeric }
                    }
                    ...
                    custom => {
                        quote! { dadagen_core::generators::CharacterSet::Custom(#custom.to_string()) }
                    }
                };
                Ok(charset)
            } else {
                Err(syn::Error::new_spanned(
                    expr,
                    "charset must be a string literal",
                ))
            }
        }
        _ => Err(syn::Error::new_spanned(
