...
fn expand_schema_definition(schema: &SchemaDefinition) -> syn::Result<TokenStream> {
    ...
    let generator_fields: Vec<_> = schema
        .fields
        .iter()
        .map(|field| {
            let field_name = &field.name;
    ...
    let field_inits: Vec<_> = schema
        ...
        .map(|field| {
            ...
            let generator_field_name = quote::format_ident!("{}_generator", field_name);
            let generator_init =
                expand_generator_config(&field.generator_config, &field_name.to_string())?;
            Ok(quote! {
    ...
    let field_generations: Vec<_> = schema
        .fields
        .iter()
        .map(|field| {
            let field_name = &field.name;
